...
$sql = "SELECT * FROM `wp_effu8c_posts` INNER JOIN `wp_effu8c_postmeta` ON `wp_effu8c_postmeta`.`post_id`=`wp_effu8c_posts`.`ID` WHERE post_author !=0 AND post_author !=1 AND guid NOT LIKE '%.png' AND `wp_effu8c_posts`.`post_title` LIKE '".$_REQUEST['cat']."%' GROUP BY `wp_effu8c_postmeta`.`post_id` ORDER BY ID DESC;";
//echo $sql;
